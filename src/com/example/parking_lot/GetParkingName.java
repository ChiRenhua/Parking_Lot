...
public class GetParkingName implements Runnable{
	...
	public GetParkingName(Context context,String url,ArrayList<ParkingInfo> parkingData,Handler handler){
		this.url = url;
		me = context;
		datas = parkingData;
	...
	private void parseJsonMsg(String str){
		ParkingInfo parking;
		try {
			JSONArray jsonObjs = new JSONObject(str)
					.getJSONArray("items");
			for (int i = 0; i < jsonObjs.length(); i++) {

				String name = ((JSONObject) jsonObjs.opt(i)).getString("key");
				parking = new ParkingInfo(me, name.substring(5, name.length()-4));//截取停车场的名字
